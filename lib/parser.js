...
parsers.status = function(gitstatus) {
  ...
  output.forEach(function(line) {
    var fileStatus = line[0]; // should just be XY
    var fileName = line[1];
    var XY = fileStatus.substring(0,2);
    var X = fileStatus.substring(0,1)
    var Y = fileStatus.substring(1,2)
    var unmerged = ['DD','AU','UD','UA','DU','AA','UU'];
    if(!fileStatus) {
      return; // empty line
    }
    if(XY == '??') {
